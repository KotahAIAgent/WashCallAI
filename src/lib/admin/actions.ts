...
const STATUS_EMAIL_CONTENT: Record<SetupStatus, { subject: string; title: string; message: string }> = {
  ...
  setting_up: {
    subject: 'Setting Up Your AI Agent - WashCall AI',
  ...
  testing: {
    subject: 'Testing Your AI Agent - WashCall AI',
  ...
  ready: {
    subject: '🎉 Your AI Agent is Ready! - WashCall AI',
  ...
  active: {
    subject: 'You\'re Live! - WashCall AI',
  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="background: white; border-radius: 12px; padding: 40px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
      ...
      <div style="text-align: center; margin: 30px 0;">
        <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://app.washcall.ai'}/app/pricing" 
    ...
    <div style="text-align: center; margin-top: 30px; color: #a1a1aa; font-size: 13px;">
      <p style="margin: 0;">WashCall AI - Never Miss Another Call</p>
    </div>
  </div>
</body>
</html>
...
${status === 'ready' 
  ? `Choose a Plan & Go Live: ${process.env.NEXT_PUBLIC_APP_URL || 'https://app.washcall.ai'}/app/pricing`
  : `Check Your Dashboard: ${process.env.NEXT_PUBLIC_APP_URL || 'https://app.washcall.ai'}/app/dashboard`
}
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      const response = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'WashCall AI <updates@washcall.ai>',
          to: customerEmail,
    ...
    try {
      const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
        ...
        body: JSON.stringify({
          ...
          from: { email: 'updates@washcall.ai', name: 'WashCall AI' },
          subject: content.subject,
          content: [

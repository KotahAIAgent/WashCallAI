...
<head>
  ...
  <title>Weekly Report - FusionCaller</title>
</head>
  ...
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="padding: 32px;">
      ...
      <p style="margin: 0 0 24px 0; color: #666;">
        Here's your weekly performance summary from FusionCaller:
      </p>
    ...
    <div style="background: #f9fafb; padding: 24px; text-align: center; border-top: 1px solid #e5e7eb;">
      <p style="margin: 0; font-size: 12px; color: #666;">
        ...
        <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://app.fusioncaller.com'}/app/settings" style="color: #3b82f6;">
          Manage preferences
        </a>
      </p>
      <p style="margin: 8px 0 0 0; font-size: 12px; color: #999;">
        © ${new Date().getFullYear()} FusionCaller. All rights reserved.
      </p>
    </div>
  </div>
</body>
...
export async function sendWeeklyReport(organizationId: string, toEmail: string) {
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'FusionCaller <reports@fusioncaller.com>',
        to: toEmail,
